...
function findMainUserObject(data, userID) {
    ...
    function deepFind(obj) {
        if (mainUserObject || typeof obj !== 'object' || obj === null)
            return;
...
function findFirstValueByKey(dataArray, key) {
    ...
    function deepSearch(obj) {
        if (found !== null || typeof obj !== 'object' || obj === null)
            return;
...
function findLiveCityFromProfileTiles(allJsonData) {
    try {
        const result = lodash_1.default.findDeep(allJsonData, (value, key, parent) => {
            return key === 'text' &&
                typeof value === 'string' &&
    ...
    return function getUserInfo(id, usePayload, callback, groupFields = []) {
        ...
        if (usePayload) {
            ...
            defaultFuncs.post("https://www.facebook.com/chat/user_info/", ctx.jar, form)
                ...
                .then(resData => {
                if (resData?.error && resData?.error !== 3252001)
                    throw resData;
                const retObj = {};
