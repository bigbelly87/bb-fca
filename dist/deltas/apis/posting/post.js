...
function default_1(defaultFuncs, api, ctx) {
    ...
    async function createPost(options, callback) {
        try {
            ...
            const postResult = await defaultFuncs.post("https://www.facebook.com/api/graphql/", ctx.jar, form, {}).then(utils.parseAndCheckLogin(ctx, defaultFuncs));
            if (postResult.error) {
    ...
    async function deletePost(postID, callback) {
        try {
            ...
            const deleteResult = await defaultFuncs.postFormData("https://www.facebook.com/api/graphql/", ctx.jar, form, {}).then(utils.parseAndCheckLogin(ctx, defaultFuncs));
                            ...
                            for (const item of payload) {
                                ...
                                for (const bboxReq of item.__bbox.require) {
                                    ...
                                    if (bboxReq[0] === "RelayPrefetchedStreamCache" && bboxReq[1] === "next") {
                                        ...
                                        if (!bboxData || !bboxData.__bbox || !bboxData.__bbox.result)
                                            continue;
