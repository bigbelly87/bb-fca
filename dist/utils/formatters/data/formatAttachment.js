...
exports._formatAttachment = _formatAttachment;
...
function _formatAttachment(attachment1, attachment2) {
    ...
    switch (type) {
        ...
        case "Sticker":
        case "StickerAttachment": return { type: "sticker", ID: blob.id, url: blob.url, packID: blob.pack?.id, spriteUrl: blob.sprite_image, spriteUrl2x: blob.sprite_image_2x, width: blob.width, height: blob.height, caption: blob.label, description: blob.label, frameCount: blob.frame_count, frameRate: blob.frame_rate, framesPerRow: blob.frames_per_row, framesPerCol: blob.frames_per_column, stickerID: blob.id };
        case "ExtensibleAttachment": {
            const story = blob.story_attachment;
            return { type: "share", ID: blob.legacy_attachment_id, url: story.url, title: story.title_with_entities.text, description: story.description?.text, source: story.source?.text, image: story.media?.image?.uri, width: story.media?.image?.width, height: story.media?.image?.height, playable: story.media?.is_playable, duration: story.media?.playable_duration_in_ms, playableUrl: story.media?.playable_url, subattachments: story.subattachments, properties: story.properties.reduce((obj, cur) => ({ ...obj, [cur.key]: cur.value.text }), {}) };
        }
...
function formatAttachment(attachments, attachmentIds, attachmentMap, shareMap) {
    ...
    return attachments ? attachments.map((val, i) => { if (!attachmentMap || !attachmentIds || !attachmentMap[attachmentIds[i]]) {
        return _formatAttachment(val);
    } return _formatAttachment(val, attachmentMap[attachmentIds[i]]); }) : [];
}
//# sourceMappingURL=formatAttachment.js.map