...
export default function(defaultFuncs: any, api: any, ctx: any) {
  ...
  async function deletePost(postID, callback) {
    try {
      ...
      if (deleteResult.error || deleteResult.errors) {
        throw new Error(
          JSON.stringify(deleteResult.error || deleteResult.errors),
        );
      }
                      ...
                      commentData.edges.forEach((edge) => {
                        ...
                        const comment: any = {
                          ...
                          author: {
                            ...
                            avatar:
                              edge.node.author?.profile_picture_depth_0?.uri,
                          },
                          reply_count:
                            edge.node.feedback?.replies_fields?.count || 0,
                          total_reply_count:
                            edge.node.feedback?.replies_fields?.total_count ||
                            0,
  ...
  async function uploadPhoto(photoPath, callback) {
    ...
    try {
      ...
      const photoId =
        uploadResult.payload?.fbid || uploadResult.payload?.photoID || null;
